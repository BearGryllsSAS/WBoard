...
// WBThumbnailProxyWidget 类继承自 QGraphicsProxyWidget，用于在图形场景中表示一个代理小部件。
// 这个类主要管理与文档代理相关的缩略图信息，并记录缩略图对应的场景索引。
// WBThumbnailProxyWidget 类扩展了 QGraphicsProxyWidget，通过引入文档代理对象和场景索引，专门用于在图形场景中表示和管理缩略图。
// 这种设计模式有效地分离了图形场景中的显示逻辑和文档管理逻辑，提高了代码的可维护性和扩展性。
class WBThumbnailProxyWidget : public QGraphicsProxyWidget
{
public:
    // 初始化 WBThumbnailProxyWidget 对象，设置文档代理对象 mDocumentProxy 和场景索引 mSceneIndex。
    ...
    // 返回当前代理小部件关联的文档代理对象 mDocumentProxy。
    WBDocumentProxy* documentProxy()
    {
        return mDocumentProxy;
    }

    // 设置缩略图对应的场景索引 mSceneIndex 为给定值 i。
    void setSceneIndex(int i)
    {
        mSceneIndex = i;
    }

    // 返回当前缩略图对应的场景索引 mSceneIndex。
    int sceneIndex()
    {
        return mSceneIndex;
    }
